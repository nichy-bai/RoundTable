...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT Bookmark_1.bookmarkID, Bookmark_1.bookmarkDate, Bookmark_1.postID, Bookmark_1.userID AS bookmarkUserID, Post.postTitle, Post.postContent, Tag.tagID, Tag.tagName, Tag.tagDesc, Topic.topicID, Topic.topicName, Topic.topicDesc, [User].name, [User].profilePicture, Post.userID AS postUserID, Post.postDate, (SELECT COUNT(*) AS Expr1 FROM DiscussionLike WHERE (postID = Post.postID) AND (likeStatus = 1)) AS totalLike, (SELECT COUNT(*) AS Expr1 FROM DiscussionComment WHERE (postID = Post.postID)) AS totalComment, (SELECT COUNT(*) AS Expr1 FROM Bookmark WHERE (postID = Post.postID)) AS totalBookmark FROM Bookmark AS Bookmark_1 INNER JOIN Post ON Bookmark_1.postID = Post.postID INNER JOIN [User] ON Bookmark_1.userID = [User].userID INNER JOIN Tag ON Post.tagID = Tag.tagID INNER JOIN Topic ON Post.topicID = Topic.topicID ORDER BY Bookmark_1.bookmarkDate DESC"></asp:SqlDataSource>
    ...
    <asp:Repeater ID="Repeater1" runat="server">
        <ItemTemplate>
            <div
                ...
                <asp:LinkButton ID="bookmarkBody_btn" runat="server" CssClass="flex flex-col cursor-pointer hover:bg-gray-50 hover:opacity-90 rounded-lg transition ease-in-out duration-300" OnCommand="bookmarkBody_btn_Command" CommandArgument='<%#Eval("postID") %>' OnClientClick="window.document.forms[0].target='_blank';">
                    <%--Discussion title--%>
                    <div class="my-5">
